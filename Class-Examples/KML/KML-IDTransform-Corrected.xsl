...
<!--ebb: Note that we need to add an XPath default namespace for KML (for input) and an xmlns for the output KML, too. All files we're processing need to be in KML format: note the two separate declarations of KML in the <xsl:stylesheet> element. (The output method has to be xml because there's no recognized "kml" value for the @method on xsl:output.)
    ...
    <xsl:variable name="PO" select="document('Placeography.kml')"/>
    <!--ebb: The variable above actually invokes a second document that we're going to pull into this transformation! It needs to be sitting in the same directory with your source KML generated by the GPS Visualizer.
        We will be able to process and mingle information from more than one file by establishing variables like this, set at the document node (or root element) of the file. We'll invoke this variable below using $PO. -->
